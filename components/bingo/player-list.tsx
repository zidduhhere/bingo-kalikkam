...
export function PlayerList({ players, currentUserId }: { players: Player[]; currentUserId: string }) {
  return (
    <div className="flex flex-col gap-4 p-4 border-[3px] border-blue-900/30 bg-white/50 backdrop-blur-sm [border-radius:225px_15px_255px_15px/15px_255px_15px_225px] rotate-[1deg] shadow-sm w-full overflow-hidden">
      ...
      <ul className="flex flex-col gap-3">
        {players.map((p) => (
          <li key={p.id} className={`flex flex-col gap-2 rounded-xl border-2 px-3 py-3 ${p.id === currentUserId ? "border-red-400/50 bg-red-50/50" : "border-blue-900/10 bg-transparent"} [border-radius:15px_255px_15px_225px/255px_15px_225px_15px]`}>
